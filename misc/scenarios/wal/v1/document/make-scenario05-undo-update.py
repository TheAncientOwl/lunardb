...
def main():
    ...
    insert_line = f'insert into Scenario_WAL_Collection objects [ {random_objects} ];'

    with open("./scenario05-undo-update.sql", "w") as scenario_file:
        scenario_file.writelines('\n'.join(lines) + '\n')
        scenario_file.write(insert_line + '\n')
        scenario_file.write(
            'update structure Scenario_WAL_Collection where ( name == Bob1 ) modify [ salary => 2500 ];')
