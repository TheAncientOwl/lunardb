...
{
    auto const query = "user create PopescuIon;";
    auto const expected = Init::UserInit{}.name("PopescuIon").action(Primitives::EUserActionType::Create);
...
TEST(Moonlight_UserParserTest, success02)
{
    ...
    auto const expected = Init::UserInit{}.name("PopescuIon").action(Primitives::EUserActionType::Remove);
